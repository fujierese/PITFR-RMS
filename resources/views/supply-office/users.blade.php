    ...
    @if($showAddUser)
            ...
            <form method="POST" action="{{ route('admin.users.store') }}" class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2" id="add-user-form">
                ...
                <div>
                    ...
                    <select id="account_type" name="account_type" required class="w-full rounded-xl border border-emerald-300 bg-white px-3 py-2 text-sm focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100">
                        @foreach(['student' => 'Student', 'outsider' => 'Outsider', 'faculty' => 'Faculty', 'student_organization' => 'Student Organization'] as $value => $label)
                            <option value="{{ $value }}" @selected(old('account_type', 'student') === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label for="add-user-name" class="mb-1 block text-sm font-medium text-slate-700">Name</label>
                    <input id="add-user-name" type="text" name="name" value="{{ old('name') }}" required maxlength="100" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input id="add-user-password" type="password" name="password" required minlength="6" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100">
                </div>
                <div>
                    <label for="add-user-password-confirmation" class="mb-1 block text-sm font-medium text-slate-700">Confirm password</label>
                    <input id="add-user-password-confirmation" type="password" name="password_confirmation" required minlength="6" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100">
                </div>
        ...
        <script>
            ...
            const updateAccountFields = () => {
                ...
                organizationFields.forEach(field => field.classList.toggle('hidden', !['outsider', 'student_organization'].includes(accountType.value)));
            };
