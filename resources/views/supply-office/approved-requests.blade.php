...
<div class="space-y-6">
    ...
    <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200/50">
        <form method="GET" action="{{ route('supply-office.requests.approved') }}" class="mb-6 grid gap-3 md:grid-cols-4">
            ...
            <a href="{{ route('supply-office.requests.approved') }}" class="rounded-xl border border-slate-300 px-4 py-2 text-center text-sm font-semibold text-slate-700 hover:bg-slate-100">Reset</a>
        </form>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($requests as $request)
                        <tr>
                            ...
                            <td class="px-4 py-4"><span class="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-700">Approved</span></td>
                            <td class="px-4 py-4 text-right">
                                ...
                                <a href="{{ route('request.print', $request->id) }}" target="_blank" class="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50">Print</a>
                            </td>
                        </tr>
                    @empty
