...
@section('content')
<div class="space-y-6">
    <x-page-header eyebrow="Requests" title="Pending Requests" description="Review incoming reservations that still need custodial and final approval." accent="slate">
    ...
    <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200/50">
        @include('supply-office.partials.request-filters', ['action' => route('supply-office.requests.pending')])
            ...
            <table class="min-w-full text-left text-sm text-slate-600">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($requests as $request)
                        <tr class="cursor-pointer transition hover:bg-slate-50 focus:bg-emerald-50 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-emerald-500" data-request-url="{{ route('request.show', $request->id) }}" role="link" tabindex="0" aria-label="Open request details for {{ $request->control_number }}">
