...
@section('content')
<div class="space-y-6">
    <div class="rounded-3xl bg-gradient-to-r from-rose-600 to-red-500 p-6 text-white shadow-sm">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
                <h1 class="text-2xl font-semibold">Rejected Requests</h1>
                <p class="mt-2 text-sm text-rose-100">Review declined reservations and their notes.</p>
            </div>
    ...
    <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200/50">
        <form method="GET" action="{{ route('supply-office.requests.rejected') }}" class="mb-6 grid gap-3 md:grid-cols-4">
            ...
            <a href="{{ route('supply-office.requests.rejected') }}" class="rounded-xl border border-slate-300 px-4 py-2 text-center text-sm font-semibold text-slate-700 hover:bg-slate-100">Reset</a>
        </form>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($requests as $request)
                        <tr>
                            ...
                            <td class="px-4 py-4"><span class="rounded-full bg-rose-100 px-2.5 py-1 text-xs font-semibold text-rose-700">Rejected</span></td>
